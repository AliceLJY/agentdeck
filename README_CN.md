...
**同一台手机是一条还是两条记录，取决于平台**：iOS 上主屏幕 App 跑在自己的 WebKit 存储里，
所以它和 Safari 持有不同的 device id、要各批一次；Android 上 Chrome 的桌面快捷方式与
...
清除站点数据会丢掉这个 id，该浏览器会以新设备身份回来，需要重新批准。iOS Safari 在存储
压力下会自己触发这个清除。

记录的 IP 只给你自己看。经过 frp tcp 隧道时，传输层的对端永远是 `127.0.0.1`，所以显示的
地址来自调用方自己设置的 `X-Forwarded-For`——只能当提示读，绝不能当来源的证明。
